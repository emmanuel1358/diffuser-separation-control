...
- **Committed-model hard contract (mandatory).** `reference_solution/` and every calibration-declared baseline (`TASK.naive`) must ship `train.py` + trained artifact(s) + `model.manifest.json` + inference-only `solution.py`. Trusted CI / ground-truth never invoke `train.py`. If the upstream task trained inside `solution.py` at grade time, split train vs inference and commit weights + digests before opening the PR. Optional `submission.csv` is Tier-B additive only. Canonical shape: [`examples/mle-tabular-classification/`](../examples/mle-tabular-classification/) and [`MLENVS_TASKS.md`](MLENVS_TASKS.md) §4.
