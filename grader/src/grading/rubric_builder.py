...
from grading.grade import Grade, _finite_clamp01
...
def _coerce_predicate_value(value: PredicateReturn) -> float:
    ...
    try:
        ...
        return 0.0
    # _finite_clamp01 (not the bare max/min idiom) so a predicate returning NaN
    # or +inf -- e.g. `pearsonr(pred, truth)[0]` on a constant prediction -- maps
    # to 0.0, not a laundered 1.0.
    return _finite_clamp01(f)
