# Alignerr Prometheus Eval CFD RL Task Guide for CFD Engineers
...
Use this eval guide with the `prometheus-eval-cfd` starter template. That starter sets `[delivery].platform = "prometheus"` and `[delivery].eval = true`. Production projects use the same Prometheus route, but should use `project_guidelines/cfd/prometheus_cfd_environments.md` and the `prometheus-cfd` starter instead.
...
to build tasks under `problems/`. This Prometheus version keeps the same CFD task
shape and trusted-CI checks as the standard CFD workflow, but routes passing
tasks to the Prometheus Agent Service runner and an independent Taiga mirror.

The eval and production Prometheus variants follow the same dual-delivery route.
...
- `[delivery]`: `platform = "prometheus"` and `eval = true` for eval Prometheus projects.
...
Every Prometheus CFD problem must include Prometheus delivery metadata and
routable task metadata:
...
## 11. Validate That the Task Challenges the Prometheus Target
...
validation, render artifact validation when declared, local agent scoring, and
score-bounds gating. If those pass, the task is exported to Harbor and submitted
to Prometheus instead of the default delivery path.

The Prometheus Eval comment reports aggregate target-model rewards across the
...
attempts complete. Solver-heavy CFD tasks can take longer because each attempt
must finish the agent episode and hidden verifier runs.
...
Prometheus export or submission happens. Error findings fail `trusted-ci/grade`,
skip downstream Prometheus delivery, and show up in the PR comment under
**Prometheus Eval — Env QA Pre-flight** with suggested fixes. The same defects,
if shipped, can still fail downstream review after the task is delivered.
...
comment; advisory warning and info findings are echoed under **Prometheus Eval**
when the Prometheus run reports. Downstream agentic QA findings are separate
from the blocking pre-flight gate and may appear after rollout metrics are
available.
