# Alignerr Prometheus Production CFD RL Task Guide for CFD Engineers
...
Use this production guide with the `prometheus-cfd` starter template. That starter sets `[delivery].platform = "prometheus"` and `[delivery].eval = false`. Eval projects use the same Prometheus route, but should use `project_guidelines/cfd/prometheus_eval_cfd_environments.md` and the `prometheus-eval-cfd` starter instead.
...
**Prometheus — Env QA Pre-flight** with suggested fixes. The same defects,
...
comment; advisory warning and info findings are echoed under **Prometheus**
