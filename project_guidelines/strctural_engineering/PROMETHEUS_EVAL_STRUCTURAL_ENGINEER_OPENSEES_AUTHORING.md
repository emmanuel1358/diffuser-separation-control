# Alignerr Prometheus Eval OpenSees RL Task Guide for Structural Engineers
...
Use this eval guide with the `prometheus-eval-structures` starter template. That starter sets `[delivery].platform = "prometheus"` and `[delivery].eval = true`. Production projects use the same Prometheus route, but should use `project_guidelines/strctural_engineering/PROMETHEUS_STRUCTURAL_ENGINEER_OPENSEES_AUTHORING.md` and the `prometheus-structures` starter instead.
...
3. Prometheus and Taiga run independently in parallel after sandbox validation.
   ...
   OpenSees availability as a native hint rather than changing your
   instruction.
...
independently weighted response criteria, hidden cases with different failure
modes, and scoring that separates formatting, feasibility, solver health,
nominal response, and robustness. Do not make the task nondeterministic just to
create spread.
...
- PR comment, section **Prometheus Eval — Env QA Pre-flight**: every finding
...
- Advisory warning and info findings: the task proceeds to Prometheus, and the
  findings are echoed under **Prometheus Eval** when the Prometheus run reports.
