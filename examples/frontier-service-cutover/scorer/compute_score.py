...
if not isinstance(request, dict) or set(request) != {"input"}:
    raise SystemExit(2)
module = runpy.run_path("app.py")
transform = module["transform"]
observed = transform(request["input"])
print(json.dumps(
    {
        "schema_version": "cutover-observation.v1",
        "observed": observed,
    },
    sort_keys=True,
    separators=(",", ":"),
...
def _validate_cases(value: Any) -> tuple[dict[str, Any], ...]:
    if (
        ...
        or value.get("schema_version") != "cutover-cases.v1"
    ):
        raise ValueError("cutover fixture has the wrong schema")
    cases = value.get("cases")
    if not isinstance(cases, list) or not 1 <= len(cases) <= 32:
        raise ValueError("cutover fixture must contain 1..32 cases")
    for case in cases:
        if (
            not isinstance(case, dict)
            or set(case) != {"record", "expected"}
            or not isinstance(case["record"], dict)
            or not isinstance(case["expected"], dict)
        ):
            raise ValueError("cutover fixture contains an invalid case")
    return tuple(cases)


def _encode_input(record: dict[str, Any]) -> bytes:
    return json.dumps(
        {"input": record},
        sort_keys=True,
        separators=(",", ":"),
...
def _validate_observation(value: Any) -> dict[str, Any]:
    if (
        ...
        or value.get("schema_version") != "cutover-observation.v1"
...
def evaluate(context):
    ...
    for index, case in enumerate(cases, start=1):
        request_bytes = context.trusted_operation(
            f"cutover case {index} request encoding",
            _encode_input,
            case["record"],
        )
        ...
        observed = context.candidate_operation(
            f"cutover case {index} observation validation",
...
TASK = RubricTask(
    ...
    fixtures={"cases": TrustedJson("cutover_cases.json")},
    criteria=(
        RubricCriterion(
            id="hidden_behavior",
            weight=0.9,
            description="Every synthetic hidden cutover record uses current state",
            required=True,
            pass_threshold=1.0,
        ),
        RubricCriterion(
