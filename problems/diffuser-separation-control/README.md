...
It declares a `RubricTask`: `JsonArtifact` owns malformed/oversized/deep JSON
and numeric overflow handling, trusted fixture declarations own private inputs,
and the shared bounded solver runner owns process-group timeouts and output
caps. `scorer/evaluation.plan.json` binds that protocol for Taiga attestation.
